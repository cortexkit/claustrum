...
The installed revision must be at least `78a8d9f` before enrolment. Before the 2026-09-19 enrolment the installed bundle was `rev=568333d`, which predates the per-account `minTtlMs` change and would have fallen back to the global 270-minute floor, producing a 90-minute period; the branch bundle (`rev=8306ec0`) was installed at 09:14Z and OpenCode restarted before the manifest was written.
...
A store change reaches a consumer's wire only on its next fetch (up to one polling interval; Insula's is ~61 s). Grade a vault-side change against the wire after a full cycle, not at write+seconds.
