...
import { OUR_PLUGIN_ID } from './handles.js'
...
function validate(input: XaiEnrollment): void {
  if (!identifierIsValid(input.provider) || !credentialIdMatchesProvider(input.credentialId, input.provider) || input.credentialId !== 'oauth:xai') throw new Error('invalid xai enrollment credential')
...
function mutateAdd(file: ManifestHandleFile, input: XaiEnrollment): void {
  ...
  if (existing && existing.accounts[0]!.handle !== input.handle) throw new Error('xai main handle differs; remove first, then re-enrol')
  if (!existing) {
