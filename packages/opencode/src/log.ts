...
import { HANDLE_FILE_CONTRACT, identifierIsValid } from "@cortexkit/claustrum-client";
...
function fieldIsSafeIdentifier(value: string): boolean {
  // Parser validity asks whether a manifest may use an identifier; this sink must independently
  // decide whether it is safe to persist. Keeping the exclusion here prevents a future parser
  // widening from silently widening the values that can reach the custody log.
  return identifierIsValid(value) && !HANDLE_FILE_CONTRACT.handleRe.test(value);
}
