...
describe("custody logger", () => {
  ...
  test("file sink excludes handle-shaped labels and providers without excluding ordinary identifiers", () => {
    ...
    // change would make this exclusion test vacuous.
    const handle = `ckh_${"a".repeat(43)}`;
    const logger = createLogger(createFileLogSink({ path }));
    logger.info({ provider: "main", label: "account-1", state: "serving" });
    logger.info({ provider: "work-alt", label: "main", state: "serving" });
    logger.info({ provider: "main", label: handle, state: "serving" });
    logger.info({ provider: handle, label: "account-1", state: "serving" });

    const contents = readFileSync(path, "utf8");
    const records = contents.trim().split("\n").map((line) => JSON.parse(line));
    expect(records.map((record) => [record.provider, record.label])).toEqual([
      ["main", "account-1"],
      ["work-alt", "main"],
      ["main", "invalid_shape"],
      ["invalid_shape", "account-1"],
    ]);
    expect(contents).not.toContain(handle);
  });
