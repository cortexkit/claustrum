...
# The current measured total is 596 (debug profile, the same command this arm
# runs). It covers master's resolved-credential-id pins and RAII temp-dir lifecycle, plus this
# branch's twelve Rust manifest-lock tests: the ABA observation that cannot rename a replacement,
# one quarantine directory per stale owner, unknown and malformed owner fields tolerated but still
# evictable, a missing nonce surfacing as owner_invalid at the deadline, and an owner that goes
# stale inside the retry window.
#
# MEASURED, NOT ARITHMETIC -- and this rebase is the second time it mattered. The branch bumped
...
# 580 and the branch 590; adding the deltas is wrong again, and master ALSO changed the command
# itself (--features credentials-core/test-support), so the two numbers were never counting the
# same set. Take master's invocation, run it on the merged tree, and read the number.
...
run_expect 596 "workspace unit + integration" \
